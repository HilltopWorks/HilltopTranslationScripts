...
tableFileName = 'ExtractTable.tbl'
injectionTableFileName = 'InjectTable.tbl'
...
def hexToText(hexObj):
    ...
    while hexConverted < hexLength:
        ...
        #Process control code if '<' encountered
        if nextMatch.startswith("3C"):
            matchBuffer = ''
            totalControlBlock = nextMatch.split('3E')[0] + '3E' #Control block ends on '>'
            matchLength = len(totalControlBlock)//2

            match = totalControlBlock.decode("hex")
        else:
...
def textToHex(textObj, tableType):
    ...
    while charsConverted < textLength:
        ...
        if nextChar == '<': #Control code processing
            symbol = textObj[charsConverted:].split('>')[0] + '>'
            for char in symbol:
                match = __findTableMatchForText__(char, asciiTable)
                returnBuffer += match.lower()
                charsConverted += 1
        elif nextChar == '{': #Custom tag processing
            symbol = textObj[charsConverted:].split('}')[0] + '}'
            match = __findTableMatchForText__(symbol, tableFile)
            returnBuffer += match.lower()
            charsConverted += len(symbol)
        elif textObj[charsConverted:charsConverted + 3] == "'m " and tableType == NORMAL_TEXT:
            symbol = textObj[charsConverted:charsConverted + 3]
            match = __findTableMatchForText__(symbol, tableFile)
            if match == NO_MATCH:
            ...
            returnBuffer += match.lower()
            charsConverted += 3
        elif nextChar == "'" and tableType == NORMAL_TEXT:
            symbol = textObj[charsConverted:charsConverted + 2]
            match = __findTableMatchForText__(symbol, tableFile)
            if match == NO_MATCH:
                match = __findTableMatchForText__(nextChar, tableFile)
                charsConverted += 1
